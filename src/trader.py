...
class Trader(arcade.Sprite):
    ...
    def update(self):
        ...
        if ((self.center_x != self.end[0] and self.center_y != self.end[1])
            and (self.center_x <= self.end[0]-END_ACCURACY and self.center_x <= self.end[0]+END_ACCURACY)
            and (self.center_y <= self.end[1]-END_ACCURACY and self.center_y <= self.end[1]+END_ACCURACY)):
